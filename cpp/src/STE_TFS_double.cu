...
// function to read the tfs table as string data - needs to be transformed TfsTableConvertStringTodouble
...
// function to convert tfs table data from string format to doubles
std::vector<std::vector<double> > STE_TFS::TfsTableConvertStringTodouble(std::vector<std::vector<std::string> > inputTable, std::map<std::string,int> requiredColumnMap)
{
	...
	for(std::vector<std::vector<std::string> >::iterator it = inputTable.begin()+1; it != inputTable.end(); ++it)
	{
		...
		for(std::map<std::string, int>::iterator jt = requiredColumnMap.begin(); jt != requiredColumnMap.end();jt++)
		{
			...
			double value = boost::lexical_cast<double>((*it)[jt->second]);
			// for debugging
...
// end function convert string table to double table
...
std::map<std::string, double>  STE_TFS::ReadTfsHeader(std::string pathFilename, bool debug)
{
	// result variable
	...
	if (infile) 
		{
			...
			for (int i=1;i<38;i++)
			{
				...
				outputmap.insert(make_pair(key, boost::lexical_cast<double>(value)));
...
void STE_TFS::Init( bool debug )
{
	...
	// transform strings table to double table and select required columns
	TFSTableData = TfsTableConvertStringTodouble(table, maptest);
...
thrust::device_vector<tfsTableData> STE_TFS::LoadTFSTableToDevice(std::vector<std::vector<double> > input)
{
...
void STE_TFS::printHeader()
{
	...
	for(std::map<std::string,double>::iterator mii = TFSHeaderData.begin(); mii != TFSHeaderData.end(); mii++)
	{
...
std::vector<std::vector<double> > STE_TFS::getTFSTableData()
{
	return TFSTableData;
}
...
std::map<std::string, double> STE_TFS::getTFSHeader()
{
